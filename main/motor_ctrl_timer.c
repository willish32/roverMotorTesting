...
static bool IRAM_ATTR motor_ctrl_timer_isr_callback(void *args)
{
    ...
    memcpy(info->pulse_info.pulse_counts, info->pulse_info.get_pulse_callback(info->pulse_info.callback_args), 6);
    //info->pulse_info.pulse_counts = info->pulse_info.get_pulse_callback(info->pulse_info.callback_args);
